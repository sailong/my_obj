...
class ListAction extends SnsController {
    ...
    public function index() {
        ...
        $this->display('sendbox');
    
    }
    
    public function sendbox() {
        echo file_get_contents(WEB_ROOT_DIR . "/Public/tool_js/sendbox/index.html");
        
    }
    
    public function submitAjax() {
        $this->ajaxReturn($_FILES, '提交成功!', 1, 'json');
    }
    ...
    public function deleteFeedAjax() {
        ...
        //删除动态对应的评论信息
        $mFeedComments = ClsFactory::Create('Model.Feed.mFeedComments');
        $comment_list = $mFeedComments->getFeedCommentsByFeedId($feed_id);
        foreach((array)$comment_list as $comment_id=>$comment) {
            $mFeedComments->delFeedComments($comment_id);
        }
