...
class PhotoComments {
    ...
    public function getPhotoCommentsById($comment_ids) {
        ...
        $comment_list = $mAlbumPhotoComments->getCommentByCommentId($comment_ids);
    ...
    public function getPhotoCommentsByPhotoId($photo_id, $offset = 0, $limit = 10) {
        ...
        $mAlbumPhotoComments = ClsFactory::Create("Model.Album.mAlbumPhotoComments");
        $comment_arr = $mAlbumPhotoComments->getAlbumPhotoCommentByPhotoId($photo_id, 1, $offset, $limit);
    ...
    public function addPhotoComments($comment_datas) {
        ...
        $mAlbumPhotoComments = ClsFactory::Create("Model.Album.mAlbumPhotoComments");
        $comment_id = $mAlbumPhotoComments->addAlbumPhotoComment($comment_datas, true);
        ...
        //更新日志对应的评论数
        $photo_id = $comment_datas['photo_id'];
        $mAlbumPhotos = ClsFactory::Create("Model.Album.mAlbumPhotos");
        $dataarr = array(
            'comments' => '%comments+1%'
        );
        $mAlbumPhotos->modifyPhotoByPhotoId($dataarr, $photo_id);
    ...
    public function delPhotoComments($comment_id) {
        ...
        //评论信息不存在或者删除评论信息本身失败
        if(empty($comment_info) || !$mAlbumPhotoComments->delCommentByCommentId($comment_id)) {
            return false;
        }
        
        //如果是一级评论，删除评论的二级评论信息
        if($comment_info['level'] == 1) {
            $child_comment_list = $mAlbumPhotoComments->getAlbumPhotoCommentByUpId($comment_id, 2);
            ...
            if(!empty($child_comment_list)) {
                foreach($child_comment_list as $child_comment_id=>$child_comment) {
                    $mAlbumPhotoComments->delCommentByCommentId($child_comment_id);
                }
            }
        }
    ...
    private function appendChildComments($comment_list) {
        ...
        $mAlbumPhotoComments = ClsFactory::Create('Model.Album.mAlbumPhotoComments');
        $child_comment_arr = $mAlbumPhotoComments->getAlbumPhotoCommentByUpId($up_ids, 2);
    ...
    */
    private function updatePhotoCommentCountByPhotoId($photo_id) {
        ...
        //获取相片评论数
        $mAlbumPhotoComments = ClsFactory::Create('Model.Album.mAlbumPhotoComments');
        $comment_count = $mAlbumPhotoComments->getCountByPhotoId($photo_id);
        //更新相片评论数
        $data_arr = array(
