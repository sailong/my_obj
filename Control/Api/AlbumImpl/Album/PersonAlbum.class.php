...
class PersonAlbum {
    ...
    protected $_mAlbumPersonGrants = null;
    protected $_AlbumInfo = null;
    
    public function __construct() {
        $this->_mAlbumPersonRelation = ClsFactory::Create('Model.Album.mAlbumPersonRelation');
        $this->_mAlbumPersonGrants = ClsFactory::Create('Model.Album.mAlbumPersonGrants');
    ...
    public function addPersonAlbum($album_data) {
        ...
        $relation_data = array(
            'album_id' => $album_id,
            'client_account' => $album_data['add_account']
        );
        ...
        $grant_id = $this->_mAlbumPersonGrants->addAlbumPersonGrant($grant_data);
        if(empty($grant_id)) {
            $this->_mAlbumPersonRelation->delAlbumPersonRelById($rel_id);
            $this->_AlbumInfo->delAlbum($album_id);
            return false;
        }
    ...
    public function getPersonAlbumByAlbumId($album_id, $client_account) {
        ...
        $album_info = $this->_AlbumInfo->getAlbum($album_id);
            //相册权限
        //三维
        $grant_list = $this->_mAlbumPersonGrants->getAlbumPersonGrantByAlbumId($album_id);
        if(!empty($grant_list)) {
    ...
    public function getPersonAlbumListByUid($client_account, $offset = null, $limit = null) {
        ...
        //相册权限
        //三维
        $grant_list = $this->_mAlbumPersonGrants->getAlbumPersonGrantByAlbumId($album_ids);
        if(!empty($grant_list)) {
    ...
    public function delPersonAlbum($album_id, $client_account) {
        ...
        //删除个人权限信息
        $this->_mAlbumPersonGrants->delByAlbumId($album_id);
    ...
    public function updPersonAlbum($album_data, $album_id, $client_account) {
        ...
        $this->_mAlbumPersonGrants->modifyAlbumPersonGrantById(array('grant'=>$album_data['grant']),$grant_info['id']);
