<?php
class JsuploadimgAction extends SnsController{
    ...
    public function index() {
        ...
        $attachDir=Pathmanagement_sns::uploadjsupload();
        $showpath = Pathmanagement_sns::getjsupload();
        ...
        if($err==''){
        	...
        	if(preg_match('/'.str_replace(',','|',$upExt).'/i',$extension))
        	{
        		...
        		else
        		{
        			...
        			$attachDirtmp = $attachDir.$attachSubDir;
        			$showpathtmp = $showpath.$attachSubDir;
        			if(!is_dir($attachDirtmp))
        			{
        				@mkdir($attachDirtmp, 0777);
        				@fclose(fopen($attachDirtmp.'/index.htm', 'w'));
        			}
        			PHP_VERSION < '4.2.0' && mt_srand((double)microtime() * 1000000);
        			$newFilename=date("YmdHis").mt_rand(1000,9999).'.'.$extension;
        			$targetPath = $attachDirtmp.'/'.$newFilename;
        			$showpathtmp .= '/'.$newFilename;
        			rename($tempPath,$targetPath);
        			@chmod($targetPath,0755);
        			$targetPath=$this->jsonString($targetPath);
        			if($immediate=='1')$targetPath='!'.$targetPath;
        			if($msgType==1)$msg="'$targetPath'";
        			else $msg="{'url':'".$showpathtmp."','localname':'".$this->jsonString($localName)."','id':'1'}";//id参数固定不变，仅供演示，实际项目中可以是数据库ID
        		}
        	}
