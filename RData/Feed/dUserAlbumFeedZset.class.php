...
class dUserAlbumFeedZset extends rBase {
    protected $zset_size = 10;
    ...
    public function getUserAlbumFeedZset($uid, $offset = 0, $limit = 10) {
        ...
        $redis_key = RedisFeedKey::getUserAblumFeedZsetKey($uid);
   ...
   /**
    ...
    public function addUserAlbumFeedZset($uid, $add_time, $feed_id) {
        ...
        //如果添加成功，修整有序集合的长度
        if($add_nums > 0) {
            $this->clipUserAlbumFeedZset($uid);
        }
    ...
    public function delUserAlbumFeedZset($uid, $feed_ids) {
        ...
        $redids_key = RedisFeedKey::getUserAblumFeedZsetKey($uid);
     ...
     * @param $class_code
     */
    private function clipUserAlbumFeedZset($uid) {
        if(empty($uid)) {
            return false;
        }
     ...
     * @param $class_code
     */
    private function isExistUserAlbumFeedZset($uid) {
        if(empty($uid)) {
            return false;
        }
    ...
    private function loader($uid) {
        if(empty($uid) || $this->isExistUserAlbumFeedZset($uid)) {
            return false;
        }
