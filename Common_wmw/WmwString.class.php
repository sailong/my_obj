...
class WmwString {
	...
	public static function keyGen() {
		return str_replace('-','',substr(WmwString::uuid(),1,-1));
	}
	...
	public static function build_format_rand($format,$number=1) {
		...
		for($j=0; $j<$number; $j++) {
			...
			for($i=0; $i<$length; $i++) {
				...
				switch($char){
					case "*"://字母和数字混合
						$strtemp   .= WmwString::rand_string(1);
						break;
					case "#"://数字
						$strtemp  .= WmwString::rand_string(1,1);
						break;
					case "$"://大写字母
						$strtemp .=  WmwString::rand_string(1,2);
						break;
					default://其他格式均不转换
						$strtemp .=   $char;
						break;
     ...
     * 检测字符串长度
     * @param  string	$string 				字符串
     * @param  int		$chiness_word_width		汉字作为的长度，默认汉字占长度为1，可选值:1或2
     */
    public static function mbstrlen($string, $chiness_word_width = 1) {
     ...
     * @param Int $length 				截取的字符串长度
     * @param Int $chiness_word_width	汉字作为的字符宽度，可选值：1或2
     * @param Boolean $append			是否截取的字符串后追加省略符号:'...'
     */
    public static function mbstrcut($string, $start, $length, $chiness_word_width = 1, $append = false) {
        $l = strlen($string);
         //截取的起始位置
        $start = max($start, 0);
        ...
        $chiness_word_width = !empty($chiness_word_width) && in_array($chiness_word_width, array(1, 2)) ? $chiness_word_width : 1;
        ...
        while($n < $l) {
            ...
            if($t == 9 || $t == 10 || (32 <= $t && $t <= 126)) {
                ...
                $tn = 3; $n += 3; $noc += $chiness_word_width;
            } elseif(240 <= $t && $t <= 247) {
                $tn = 4; $n += 4; $noc += 1;
            } elseif(248 <= $t && $t <= 251) {
                $tn = 5; $n += 5; $noc += 1;
	...
	public static function getfirstchar($s0){
		...
		if( ($s = iconv("UTF-8","gb2312//IGNORE", $s0)) !== false ){
	...
	public static function getNumsUppercase($intNums) {
		switch($intNums) {
			case 1 :
				$outStr = "一";
			break;
	...
	public static function operatorArr($opernum){
		...
		foreach($opernum as $key=>&$val){
			if(in_array($val,$moveArr)){
				$oper[1] = '1';
			}elseif(in_array($val,$unicomArr)){
				$oper[2] = '2';
			}elseif(in_array($val,$telecomArr)){
				$oper[3] = '3';
			}
		}
		//键值递增排序
		ksort($oper);
		//将数组变成字符串
		$operstr=implode("",$oper);
		unset($oper);
		$return_arr = array();
		switch ($operstr){
			case '1':
				$return_arr = $moveNumArr;
				break;
			case '2':
				$return_arr = $unicomNumArr;
				break;
			case '3':
				$return_arr = $telecomNumArr;
				break;
			case '12':
				$return_arr = array_merge($moveNumArr,$unicomNumArr);
				break;
			case '13':
				$return_arr = array_merge($moveNumArr,$telecomNumArr);
				break;
			case '23':
				$return_arr = array_merge($unicomNumArr,$telecomNumArr);
				break;
			case '123':
				$return_arr = array_merge($moveNumArr,$unicomNumArr,$telecomNumArr);
				break;
			default:
				return false;
				break;
