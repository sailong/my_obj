...
create_album.prototype.attachEvent=function(){
	...
	//绑定form的提交事件
	$('#create_album_form', context).submit(function() {
		...
		$(this).ajaxSubmit({
			...
			success:function(json) {
				...
				$.ajax({
					type:"get",
					url:me.get_album_url + '/album_id/' + json.data,
					dataType:"json",
					async:false,
					success:function(json) {
	...
	$('#sub_btn',  context).click(function() {
		$('#create_album_form', context).submit();
	});
	...
	$('#create_album_div').bind({
		openEvent: function(evt, options) {
			...
			art.dialog({
				...
				init:function() {
					//post提交路径
					$('#create_album_form',$('#create_album_div')).attr('action',options.add_post_url);
					me.get_album_url = options.get_album_url;
					me.clearInfo();
...
create_album.prototype.fillClassGrantList=function(options) {
	options = options || {};
	var me = this;
...
create_album.prototype.initFormOptions=function(options) {
	var me = this;
	options = options || {};
	var formObj = $('#create_album_form', $('#create_album_div'));
	//清楚已有的自定义隐藏域
