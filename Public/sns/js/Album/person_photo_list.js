...
photo_list.prototype.attachEvent = function(){
	...
	$("#upd_album_btn", $(".list_photo_right")).click(function() {
		$('#edit_album_div').trigger('openEvent',[{
			upd_post_url:'/Sns/Album/Personalbum/updAlbum',//修改相册路径
			get_grant_url:'/Sns/Album/Personalbum/getGrantList',//获取相册权限路径
			get_album_url:'/Sns/Album/Personalbum/getAlbum/client_account/'+me.client_account+'/album_id/'+me.album_id,
			add_form_info:{client_account:me.client_account,album_id:me.album_id},
...
photo_list.prototype.updPhotoName=function(photo_id,photo_name){
	...
	$.ajax({
		...
		dataType:"json",
		url:"/Sns/Album/Personphoto/updPhotoName",
		success:function(json) {
			if(json.status < 0) {
				isTrue = false;
			}
...
photo_list.prototype.deleteAlbum=function() {
	...
	$.ajax({
		type:"get",
		dataType:"json",
		url:"/Sns/Album/Personalbum/delAlbum/client_account/" + me.client_account + "/album_id/" + me.album_id,
		async:false,
		success:function(json) {
			if(json.status < 0) {
				$.showError(json.info);
			...
			window.location.href = "/Sns/Album/Personalbum/albumlist/client_account/" + me.client_account;
		}
	});
};
