function sendCommentBox(sendOptions){
	...
	var paramData = {"photo_id":photo_id,"up_id":up_id};
...
};//http://vm1.wmw.cn/Sns/Album/Photocomments/getCommentsList/photo_id/1172/page/1
//删除评论
...
class_show.prototype = {
	...
	loadDatas:function(photo_id,page) {
		var me = this;
		page = page || 1;
		photo_id = photo_id || {};
		var is_success = true;
		$.ajax({
			type:'get',
			url:"/Sns/Album/Photocomments/getCommentsList/photo_id/"+photo_id+"/page/"+page,
			dataType:'json',
			async:false,
			success:function(json) {
				if(json.status < 0) {
