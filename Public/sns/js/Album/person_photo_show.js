...
photoShowPer.prototype.init=function() {
	var self = this;
	$('#xpxq_div').trigger('openEvent', [{
		...
		callback_url:'/Sns/Album/Personphoto/getPhotoInfoByPhotoId',
		is_edit:self.is_edit
	}]);
};
photoShowPer.prototype.delegateEvent=function() {
	var self = this;
	//移动相片
...
photoShowPer.prototype.reflushCounter=function() {
	var self = this;
...
//设为封面
photoShowPer.prototype.setAlbumImg=function(obj) {
	var self = this;
	var dlObj = obj || {};
	...
	$.ajax({
		type:"post",
		data:{'album_id':self.album_id,'album_img':album_img,'client_account':self.client_account},
		dataType:"json",
		async:false,
		url:'/Sns/Album/Personalbum/setAlbumImg',//"/Api/Album/setAlbumImgByClass",
		success:function(json) {
			if(json.status<0) {
				$.showError(json.info);
				return false;
...
//删除相片
photoShowPer.prototype.delPhoto=function(obj) {
	var self = this;
	var ancestorOb = obj;
	...
	$.ajax({
		type:"get",
		dataType:"json",
		url:'/Sns/Album/Personphoto/delPhoto/photo_id/'+photo_id,
		async:true,
		success:function(json) {
			if(json.status < 0) {
				$.showError(json.info);
...
//照片移动//ok
photoShowPer.prototype.getAlbumList=function() {
	var self = this;
	...
	if($.isEmptyObject(album_list_tmp)) {
		$.ajax({
			type:"get",
			dataType:"json",
			url:"/Sns/Album/Personphoto/getAlbumList/client_account/"+self.client_account,
			async:false,
			success:function(json) {
				if(json.status < 0) {
					$.showError(json.info);
...
//ok
photoShowPer.prototype.fillAlbumList = function (album_list) {
	var self = this;
...
//移动照片
photoShowPer.prototype.movePhoto=function(to_album_id, photoData) {
	var self = this;
	...
	$.ajax({
		...
		dataType:"json",
		url:"/Sns/Album/Personphoto/movePhoto",
		async:true,
		success:function(json) {
			if(json.status < 0) {
				$.showError(json.info);
