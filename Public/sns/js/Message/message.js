...
Message.prototype.clear_msg = function(){
	var me = this;
	$("#homework,#exam,#notice,#privatemsg").live('click', function(){
		var uid = $("#uid").val();
		me.socket.emit('clear_msg', uid , this.id);
		var obj_str = 'is_pull_' + this.id;
		me[obj_str] = false;		
	});
};
...
Message.prototype.show_message=function(){
	...
	var exam_num = $("#exam_num").html().replace(/[^0-9]/ig, "") != 0  && self.is_pull_exam ? $("#show_msg_load_main").append('<p><span>' + $("#exam_num").html().replace(/[^0-9]/ig, "")  + '条</span>班级成绩<a href="/Sns/ClassExam/Exam/index/class_code/' + class_code +'" class="f_orange" id="exam">查看</a></p>') : 0;
